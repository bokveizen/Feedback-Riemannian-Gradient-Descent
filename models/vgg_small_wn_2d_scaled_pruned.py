...
from conv_decomp import Conv2dWNScaledPruned, Conv2dWNScaledPrunedWithBN
...
def make_layers(cfg, batch_norm=False, args=None):
    ...
    for l in cfg:
        ...
        # layers += [Conv2dWN(input_channel, l, kernel_size=3, padding=1, args=args)]
        #
        # if batch_norm:
        #     layers += [nn.BatchNorm2d(l)]
        if batch_norm:
            layers += [Conv2dWNScaledPrunedWithBN(input_channel, l, kernel_size=3, padding=1, args=args)]
        else:
            layers += [Conv2dWNScaledPruned(input_channel, l, kernel_size=3, padding=1, args=args)]
