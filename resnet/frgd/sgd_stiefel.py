...
class SGDStiefel(Optimizer):
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0, weight_decay=0, nesterov=False,
                 oblique=0., stiefel=0., feedback=(3, 3)):
                    ...
                    if p.dim() == 2:  # FC weight
                        ...
                        if p.shape[0] <= p.shape[1] and stiefel > 0:  # Stiefel
                            ...
                            if momentum != 0:  # Riemannian momentum
                                ...
                                if 'momentum_buffer' not in param_state:  # v0
                                    ...
                                    rd.add_(buf, alpha=momentum - 1).add_(d_p, alpha=-lr)
                                    q = rd.mm(p.data.t())
                                    q = q.add(q.t()).mul(0.5)
                                ...
                                fb = torch.zeros_like(p.data)
                                if feedback[0] > 0:
                                    fb.sub_(p.data.mm(p.data.t()).mm(p.data).sub(p.data), alpha=feedback[0])
                                ...
                                if feedback[0] > 0:
                                    d_p.sub_(p.data.mm(p.data.t()).mm(p.data).sub(p.data), alpha=feedback[0])
                            ...
                            if momentum != 0:  # Riemannian momentum
                                ...
                                fb = torch.zeros_like(p.data)
                                if feedback[0] > 0:
                                    fb.sub_(p.data.norm(dim=1).pow(2).sub(1).unsqueeze(-1).mul(p.data),
                                            alpha=feedback[0])
                                # if feedback[1] > 0:
                                #     fb.sub_(feedback[1] * lr,
                                ...
                                d_p.sub_(torch.bmm(d_p.unsqueeze(1), p.data.unsqueeze(2)).flatten()
                                         ...
                                         .mul(p.data))
                                if feedback[0] > 0:
                                    d_p.sub_(p.data.norm(dim=1).pow(2).sub(1).unsqueeze(-1).mul(p.data),
                                             alpha=feedback[0])
                                # if not any(feedback) and weight_decay != 0:
                                #     d_p.add_(weight_decay, p.data)
                                p.data.add_(d_p)
                        ...
                        if p_2d.shape[0] <= p_2d.shape[1] and stiefel > 0:  # Stiefel
                            ...
                            if momentum != 0:  # Riemannian momentum
                                ...
                                fb_2d = fb.view_as(p_2d)
                                if feedback[0] > 0:
                                    fb_2d.sub_(p_2d.mm(p_2d.t()).mm(p_2d).sub(p_2d), alpha=feedback[0])
                                # if feedback[1] > 0:
                                ...
                                if feedback[0] > 0:
                                    d_p_2d.sub_(p_2d.mm(p_2d.t()).mm(p_2d).sub(p_2d),
                                                alpha=feedback[0])
                                # if not any(feedback) and weight_decay != 0:
                                #     d_p.add_(weight_decay, p.data)
                                p.data.add_(d_p)
                            ...
                            if momentum != 0:  # Riemannian momentum
                                ...
                                if 'momentum_buffer' not in param_state:  # v0
                                    ...
                                    rd = torch.zeros_like(p.data)
                                    rd_2d = rd.view_as(p_2d)
                                    rd.add_(buf, alpha=feedback[0]).add_(d_p, alpha=-lr)
                                ...
                                fb_2d = fb.view_as(p_2d)
                                if feedback[0] > 0:
                                    fb_2d.sub_(p_2d.norm(dim=1).pow(2).sub(1).unsqueeze(-1).mul(p_2d),
                                               alpha=feedback[0])
                                # if feedback[1] > 0:
                                #     fb_2d.sub_(feedback[1] * lr,
                                ...
                                d_p_2d.sub_(torch.bmm(d_p_2d.unsqueeze(1), p_2d.unsqueeze(2)).flatten()
                                            ...
                                            .mul(p_2d))
                                if feedback[0] > 0:
                                    d_p_2d.sub_(p_2d.norm(dim=1).pow(2).sub(1).unsqueeze(-1).mul(p_2d),
                                                alpha=feedback[0])
                                # if not any(feedback) and weight_decay != 0:
                                #     d_p.add_(weight_decay, p.data)
                                p.data.add_(d_p)
