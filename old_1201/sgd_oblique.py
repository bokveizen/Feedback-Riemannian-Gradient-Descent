                    ...
                    if p.dim() == 2:  # FC weight
                        if momentum != 0:  # Riemannian momentum
                            ...
                            if self.feedback[0] > 0:
                                fb.sub_(self.feedback[0], p.data.norm(dim=1).pow(2).sub(1).unsqueeze(-1).mul(p.data))
                            ...
                            d_p.sub_(torch.bmm(d_p.unsqueeze(1), p.data.unsqueeze(2)).flatten()
                                     ...
                                     .mul(p.data))
                            if self.feedback[0] > 0:
                                d_p.sub_(self.feedback[0], p.data.norm(dim=1).pow(2).sub(1).unsqueeze(-1).mul(p.data))
                            p.data.add_(d_p)
                        ...
                        if momentum != 0:  # Riemannian momentum
                            ...
                            d_p_2d.sub_(torch.bmm(d_p_2d.unsqueeze(1), p_2d.unsqueeze(2)).flatten()
                                        ...
                                        .mul(p_2d))
                            if self.feedback[0] > 0:
                                d_p_2d.sub_(self.feedback[0], p_2d.norm(dim=1).pow(2).sub(1).unsqueeze(-1).mul(p_2d))
                            p.data.add_(d_p)
        return loss
